...
import { Send, BookOpen, UserCircle2 } from 'lucide-react';
...
export function JournalWidget() {
  ...
  const handleSave = async () => {
    ...
    const fullContent = narasumber.trim() 
      ? `Narasumber: ${narasumber.trim()}\n\n${content.trim()}`
      : content.trim();
    ...
    <div className="relative flex-1 flex flex-col gap-4 z-10">
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Nama Narasumber / Pembicara (Opsional)..."
          className="w-full bg-white rounded-2xl pl-11 pr-5 py-3 text-sm text-dark font-medium placeholder:font-medium placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-primary/50 border border-slate-100 transition-all hover:border-primary/30"
        />
      </div>
